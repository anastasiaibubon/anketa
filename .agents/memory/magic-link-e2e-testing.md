...
**Quota warning:** each run of this technique sends one real email via Firebase's `accounts:sendOobCode`, which counts against the project's daily email-sign-in quota shared with real users. Two automated sends in one verification session (plus a handful of manual ones) were enough to trip `QUOTA_EXCEEDED` on anketa's project. Do not repeat this live-send flow for routine/incremental re-checks — reuse the `idToken`/`refreshToken` from one sign-in for the rest of the session (refresh via `securetoken.googleapis.com` instead of re-sending), and only trigger a fresh `sendOobCode` when a new session is genuinely required.
